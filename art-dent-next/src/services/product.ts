...
const unwrapObj = (res: any) => res?.data?.data ?? res?.data ?? res;

/** API → UI */
export type UIProduct = {
  idArticulo: number | string | null;
...
const toUI = (p: any = {}): UIProduct => ({
  ...
  Activo: p.Activo != null ? Number(p.Activo) === 1 : !!(p.is_active ?? true),
...
const ONE_PATHS = (id: string | number) => [`/products/${id}`];

const CAT_PATHS = ["/categories"];
const TAX_PATHS = ["/taxes"];

const IMG_LIST_PATHS = (id: string | number) => [`/products/${id}/images`];
const IMG_PRIMARY_PATHS = (id: string | number, imageId: string | number) => [
  `/products/${id}/images/${imageId}/primary`,
];
const IMG_DELETE_PATHS = (id: string | number, imageId: string | number) => [
  `/products/${id}/images/${imageId}`,
];

/** Helpers */
const firstOkGET = async (paths: string[], params?: any) => {
  let lastErr: any;
...
const firstOkPOST = async (paths: string[], body?: any, cfg?: any) => {
  let lastErr: any;
  for (const p of paths) {
    try {
...
const firstOkPUT = async (paths: string[], body?: any, cfg?: any) => {
  let lastErr: any;
  for (const p of paths) {
    try {
...
const firstOkDELETE = async (paths: string[]) => {
  let lastErr: any;
  for (const p of paths) {
    try {
...
export const listProducts = async (params: Record<string, any> = {}) => {
  ...
  return unwrapRows(r).map(toUI) as UIProduct[];
};
...
export const toggleProductActive = async (id: string | number) => {
  const cur = await getProduct(id);
  return updateProduct(id, { is_active: !cur.Activo });
};
...
export const listProductImages = async (id: string | number) => {
...
export const uploadProductImage = async (
  id: string | number,
  file: File,
  { alt = "", is_primary = false, sort_order = 0 }: { alt?: string; is_primary?: boolean; sort_order?: number } = {}
) => {
...
export const setProductImagePrimary = async (id: string | number, imageId: string | number) => {
  const r = await firstOkPOST(IMG_PRIMARY_PATHS(id, imageId));
  return unwrapObj(r);
};

export const deleteProductImage = async (id: string | number, imageId: string | number) => {
