...
import EditarArticulo from './EditarArticulo';
 // Asegúrate de importar getIdBaseFromToken
...
const Articulos = () => {
  ...
  const handleToggle = async (id) => {
    try {
        await Products.toggleProductActive(id);
        fetchArticulos();
    } catch (error) {
  ...
  const handleOpenEditar = async (articulo) => {
    try {
      const articuloCompleto = await Products.getProduct(articulo.idArticulo);
  ...
  const fetchArticulos = async () => {
    try {
      // Obtener el idBase del token
      const articulos = await Products.listProducts(); // Pasar idBase a tu función
