...
export default function Header() {
  ...
  const megaMenuData = useMemo(() => {
    // Filtrar categorías principales (sin parent_id)
    const mainCategories = categories.filter((cat) => !cat.parent_id)
    
    // Organizar en columnas (máximo 3 columnas)
  ...
  // Mega menú estático para marcas (mantener el original)
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      {/* Header principal (estética tipo Olympic) */}
      ...
      >
        <div className="mx-auto max-w-7xl px-4">
          <nav className="relative flex items-center justify-center gap-6 py-2 text-sm font-semibold">
            ...
            {/* Productos (mega menu con categorías dinámicas) */}
            <Link
              to="/productos"
