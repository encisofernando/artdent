...
.row .label { display: table-cell; width: 45%; font-size: 7.5pt; color: #7A8A9A; text-transform: uppercase; font-weight: 700; }
.row .value { display: table-cell; width: 55%; font-size: 9.5pt; font-weight: 600; text-align: right; }
</style>
</head>
<body>
...
@php
    ...
    $numero = str_pad((string) $move->id, 8, '0', STR_PAD_LEFT);
    $isCredit = in_array($move->type, ['payment', 'note_credit']);
@endphp
    ...
    <table>
        <tr>
            ...
            <td style="width: 42%; padding: 8px 10px; vertical-align: top;">
                <div style="font-size: 15pt; font-weight: 900; color: #397B9C; letter-spacing: -0.3px;">COMPROBANTE DE CUENTA</div>
                <div style="font-size: 6.8pt; color: #888; margin-bottom: 4px;">Comprobante interno de cuenta corriente — no válido como factura.</div>
                <div style="font-size: 7.5pt;">
                    <strong>N°:</strong> {{ $numero }} &nbsp;·&nbsp; <strong>Fecha:</strong> {{ $fmtDate($move->move_date) }}
                </div>
            </td>
        </tr>
    </table>
...
<div class="card">
    <div class="card-header">Odontólogo</div>
    <div class="card-body">
...
<div class="card">
    <div class="card-header">Detalle del movimiento</div>
    <div class="card-body">
        <div class="row"><span class="label">Tipo</span><span class="value">{{ $moveLabel }}</span></div>
        <div class="row"><span class="label">Descripción</span><span class="value">{{ $move->description ?: '—' }}</span></div>
        <div class="row"><span class="label">Monto</span><span class="value" style="color: {{ $isCredit ? '#3D8B6B' : '#B23A3A' }};">{{ $isCredit ? '−' : '+' }} $ {{ $fmt($move->amount) }}</span></div>
        <div class="row"><span class="label">Saldo tras el movimiento</span><span class="value">$ {{ $fmt($move->balance_after) }}</span></div>
    </div>
</div>
