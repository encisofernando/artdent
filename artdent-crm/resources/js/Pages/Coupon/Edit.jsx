...
// Convert "2025-03-15T00:00:00.000Z" → "2025-03-15T00:00" for datetime-local input
...
export default function Edit({ auth, item }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        max_uses_per_customer: item.max_uses_per_customer ?? '',
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('coupons.update', item.id));
    };
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title={`Editar Cupón: ${item.code}`} />

            <div className="flex flex-col gap-6 font-sans max-w-4xl mx-auto">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        <h1 className={`text-2xl font-extrabold tracking-tight font-mono ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{item.code}</h1>
                        <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Editando cupón de descuento</p>
                    </div>
                ...
                {/* Stats de uso */}
                <div className={`rounded-2xl border p-5 ${isDark ? 'bg-slate-900 border-slate-700/60' : 'bg-white border-slate-100'}`}>
                    <div className={`flex items-center gap-2 mb-4 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                        <BarChart2 size={16} style={{ color: B.teal }} />
                        <span className="text-xs font-bold uppercase tracking-wider">Estadísticas de Uso</span>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {[
                            { label: 'Usos totales', value: item.used_count ?? 0 },
                            { label: 'Límite', value: item.max_uses ?? '∞' },
                            { label: 'Válido desde', value: fmtDate(item.valid_from) },
                            { label: 'Válido hasta', value: fmtDate(item.valid_until) },
                        ].map(s => (
                            <div key={s.label} className={`rounded-xl p-3 text-center ${isDark ? 'bg-slate-800' : 'bg-slate-50'}`}>
                                <div className={`text-xl font-black ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{s.value}</div>
                                <div className={`text-[10px] font-medium mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{s.label}</div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="flex flex-col gap-6">
                    ...
                    <div className={section}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label className={lbl}>{data.type === 'percentage' ? 'Porcentaje *' : 'Monto *'}</label>
                                <div className="relative">
                            ...
                            <div>
                                <label className={lbl}>Pedido Mínimo</label>
                                <div className="relative">
                    ...
                    <div className={section}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" value={data.valid_from} onChange={e => setData('valid_from', e.target.value)} className={inp} />
                                {errors.valid_from && <div className={err}>{errors.valid_from}</div>}
                            </div>

                            <div>
                                <label className={lbl}>Válido Hasta</label>
                                <input type="datetime-local" value={data.valid_until} onChange={e => setData('valid_until', e.target.value)} className={inp} />
