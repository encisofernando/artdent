...
import { Plus, Search, Trash2, Pencil, X, Save, Percent } from 'lucide-react';
...
export default function Index({ auth, items, employees, filters, summary }) {
    ...
    const applyFilters = () => router.get(route('employee-discounts.index'), { employee_id: empFilter || undefined, search: search || undefined, from: from || undefined, to: to || undefined }, { preserveState: true, replace: true });
    const clearFilters = () => { setEmpFilter(''); setSearch(''); setFrom(''); setTo(''); router.get(route('employee-discounts.index')); };
    ...
    const submitForm = (e) => {
        e.preventDefault();
        setProcessing(true);
        const url = editing ? route('employee-discounts.update', editing.id) : route('employee-discounts.store');
    ...
    const handleDelete = (id) => { confirmDialog('¿Eliminar este descuento?', () => router.delete(route('employee-discounts.destroy', id), { preserveScroll: true })); };
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Descuentos de Personal" />
            <div className="flex flex-col gap-6 font-sans max-w-5xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className={`text-2xl font-extrabold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>Descuentos</h1>
                            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{summary.records} registros · Total: ${fmt(summary.amount)}</p>
                        </div>
                    </div>
                    <button onClick={openCreate} className="inline-flex items-center gap-2 px-4 py-2.5 min-h-[40px] rounded-xl text-sm font-bold text-white" style={{ background: 'linear-gradient(90deg, #9C3939, #9C6149)' }}>
                ...
                <div className="sm:hidden flex flex-col gap-3">
                    {items.data.length === 0 ? (
                        ...
                        <div key={item.id} className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-slate-900 border-slate-700/60' : 'bg-white border-slate-200 shadow-sm'}`}>
                            <div style={{ height: 3, background: 'linear-gradient(90deg, #9C3939, #9C6149)' }} />
                            <div className="p-4">
                                <div className="flex items-start justify-between gap-3 mb-2">
                                    ...
                                    <p className={`font-extrabold text-base shrink-0 ${isDark ? 'text-red-400' : 'text-red-600'}`}>−${fmt(item.amount)}</p>
                                </div>
                            ...
                            <tbody className={`divide-y ${isDark ? 'divide-slate-800' : 'divide-slate-100'}`}>
                                {items.data.length === 0 ? (
                                    ...
                                    <tr key={item.id} className={`${isDark ? 'hover:bg-slate-800/40' : 'hover:bg-slate-50'}`}>
                                        ...
                                        <td className={`px-4 py-3 font-bold ${isDark ? 'text-red-400' : 'text-red-600'}`}>−${fmt(item.amount)}</td>
            ...
            <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Editar Descuento' : 'Nuevo Descuento'} isDark={isDark}>
                <form onSubmit={submitForm} className="flex flex-col gap-4">
                    ...
                    <div><label className={labelCls}>Concepto *</label><input type="text" value={form.concept} onChange={e => setForm(f => ({ ...f, concept: e.target.value }))} className={inputCls} placeholder="Adelanto, Falta, Multa..." required /></div>
                    <div className="flex justify-end gap-2">
                        ...
                        <button type="submit" disabled={processing} className="inline-flex items-center gap-2 px-5 py-2.5 min-h-[40px] rounded-xl text-sm font-bold text-white disabled:opacity-50" style={{ background: 'linear-gradient(90deg, #9C3939, #9C6149)' }}><Save size={14} /> Guardar</button>
                    </div>
                </form>
            </Modal>
        </AuthenticatedLayout>
