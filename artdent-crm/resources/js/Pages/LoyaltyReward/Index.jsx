...
import { Search, Plus, Edit, Trash2, Gift, CheckCircle2, XCircle } from 'lucide-react';
...
export default function Index({ auth, items, filters }) {
    ...
    useEffect(() => {
        ...
        if (changed) {
            router.get(route('loyalty-rewards.index'), { search: debouncedSearch, status }, {
    ...
    const handleDelete = (id) => {
        confirmDialog('¿Eliminar esta recompensa? Esta acción no se puede deshacer.', () => {
            router.delete(route('loyalty-rewards.destroy', id), { preserveScroll: true });
        });
    };

    const fmtMoney = (n) => `$${Number(n || 0).toLocaleString('es-AR')}`;
    const fmtPoints = (n) => `${Number(n || 0).toLocaleString('es-AR')} pts`;
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Recompensas de Puntos" />
            ...
            <div className="flex flex-col gap-6 font-sans">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                                Cuántos puntos hacen falta para cada descuento — se usan en el POS y en el checkout online
                            </p>
                        </div>
                    </div>
                    <Link href={route('loyalty-rewards.create')}>
                ...
                {data.length === 0 ? (
                    <div className={`${card} p-12 text-center`}>
                        <Gift size={40} className={`mx-auto mb-3 ${isDark ? 'text-slate-600' : 'text-slate-300'}`} />
                        <p className={`font-semibold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>No hay recompensas cargadas</p>
                        <p className={`text-sm mt-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Creá la primera para que los clientes puedan canjear puntos.</p>
                    </div>
                ) : (<>
                    {/* Mobile cards */}
                                ...
                                <div className="p-4">
                                    <div className="flex items-start justify-between gap-3 mb-2">
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{fmtPoints(item.points_cost)}</p>
                                        </div>
                                        <div className="text-right shrink-0">
                                            <p className={`font-bold text-sm ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>{fmtMoney(item.discount_amount)}</p>
                                            {item.is_active ? (
                                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${isDark ? 'bg-emerald-900/30 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}><CheckCircle2 size={10} /> Activa</span>
                                            ) : (
                                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${isDark ? 'bg-red-900/30 text-red-400' : 'bg-red-50 text-red-600'}`}><XCircle size={10} /> Inactiva</span>
                                            )}
                                        </div>
                                    </div>
                                    <div className={`flex items-center justify-end pt-2 border-t ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
                                        <div className="flex gap-1.5">
                                            <Link href={route('loyalty-rewards.edit', item.id)}>
                                        ...
                                        <tr key={item.id} className={`transition-colors ${isDark ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'}`}>
                                            <td className={`px-4 py-3 font-semibold ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                                                <div className="flex items-center gap-2">
                                                    <Gift size={14} className="text-teal-500 shrink-0" />
                                                    {item.name}
                                                </div>
                                            </td>
                                            <td className={`px-4 py-3 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{fmtPoints(item.points_cost)}</td>
                                            <td className={`px-4 py-3 font-semibold ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>{fmtMoney(item.discount_amount)}</td>
