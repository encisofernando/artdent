...
function AttendanceForm({ form, employees, isEdit = false, isDark }) {
        ...
        <div className="flex flex-col gap-4">
            ...
            <label className={`flex items-center gap-2 text-sm font-semibold ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                <input
                    ...
                    onChange={e => form.setData('is_absent', e.target.checked)}
                    className="rounded"
                />
                Ausencia
            </label>
            ...
            {form.data.is_absent ? (
                <div>
                    <label className={labelClass}>Motivo de ausencia</label>
                    <input type="text" className={inputClass} value={form.data.absence_reason} onChange={e => form.setData('absence_reason', e.target.value)} placeholder="Ej. Enfermedad, licencia..." />
                </div>
            ) : (
...
export default function Index({ auth, items, employees, filters, summary }) {
    ...
    const createForm = useForm({ employee_id: '', work_date: today, time_in: '', time_out: '', is_absent: false, absence_reason: '', method: 'manual', notes: '' });
    const editForm = useForm({ work_date: '', time_in: '', time_out: '', is_absent: false, absence_reason: '', method: 'manual', notes: '' });
    ...
    }, [employeeFilter, from, to, filters?.employee_id, filters?.from, filters?.to]);
    ...
    const submitCreate = (e) => {
        e.preventDefault();
        createForm.post(route('employee-attendances.store'), {
    ...
    const submitEdit = (e) => {
        e.preventDefault();
        editForm.put(route('employee-attendances.update', editItem.id), {
    ...
    const handleDelete = (id) => {
        confirmDialog('¿Eliminar este registro de asistencia?', () =>
            router.delete(route('employee-attendances.destroy', id), { preserveScroll: true })
        );
    };
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Asistencias de Empleados" />
            ...
            <div className="flex flex-col gap-6 font-sans">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Control horario de empleados</p>
                        </div>
                    </div>
                    {canCreate && (
                        <Button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        ...
                        { label: 'Ausencias', value: summary?.absences ?? 0 },
                    ].map(({ label, value }) => (
                        <div
                            key={label}
                ...
                <div className={`flex flex-wrap items-center gap-3 p-4 rounded-2xl border ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'}`}>
                    ...
                    {(employeeFilter || from || to) && (
                        <button onClick={() => { setEmployeeFilter(''); setFrom(''); setTo(''); }} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1">
                            <X size={14} /> Limpiar
                        </button>
                    )}
                    {items?.total > 0 && (
                                ...
                                <div key={item.id} className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-slate-900 border-slate-700/60' : 'bg-white border-slate-200 shadow-sm'}`}>
                                    ...
                                    <div className="p-4">
                                        <div className="flex items-start justify-between gap-3 mb-2">
                                            ...
                                            {item.is_absent ? (
                                                <span className="px-2 py-0.5 rounded-lg text-xs font-bold shrink-0 bg-red-500/10 text-red-500">
                                                    Ausente
                                                </span>
                                            ) : (
                                                <span className={`px-2 py-0.5 rounded-lg text-xs font-bold shrink-0 ${METHOD_COLORS[item.method] || 'bg-slate-500/10 text-slate-400'}`}>
                                        ...
                                        <div className={`flex items-center justify-between pt-3 border-t ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
                                            {item.is_absent ? (
                                                <span className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{item.absence_reason || '—'}</span>
                                            ) : (
                                                <span className={`text-sm font-mono ${isDark ? 'text-green-400' : 'text-green-700'}`}>
                                                    {(item.time_in || '').substring(0, 5) || '—'}
                                                    <span className={`mx-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>→</span>
                                                    <span className={item.time_out ? (isDark ? 'text-red-400' : 'text-red-600') : (isDark ? 'text-slate-600' : 'text-slate-400')}>
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead className={`border-b ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                                        <tr>
                                            {['Empleado', 'Fecha', 'Entrada', 'Salida', 'Horas', 'Método', ...(showActions ? [''] : [])].map(h => (
                                                ...
                                                {item.is_absent ? (
                                                    <td colSpan={2} className="px-4 py-3">
                                                        <span className="px-2 py-0.5 rounded-lg text-xs font-bold bg-red-500/10 text-red-500">
                                                            Ausente{item.absence_reason ? ` — ${item.absence_reason}` : ''}
                                                        </span>
                                                    </td>
                                                ) : (
                                                    <>
            ...
            {showCreate && canCreate && (
                <Modal title="Nuevo Registro de Asistencia" onClose={() => setShowCreate(false)}>
                    <form onSubmit={submitCreate}>
                        <AttendanceForm form={createForm} employees={employees} isDark={isDark} />
            ...
            {editItem && canEdit && (
                <Modal title="Editar Asistencia" onClose={() => setEditItem(null)}>
                    <div className={`mb-4 px-3 py-2.5 min-h-[40px] rounded-xl text-sm font-semibold ${isDark ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-700'}`}>
                        {editItem.employee?.user?.name}
                    </div>
                    <form onSubmit={submitEdit}>
                        <AttendanceForm form={editForm} employees={employees} isEdit isDark={isDark} />
                        <div className="flex gap-3 mt-6">
