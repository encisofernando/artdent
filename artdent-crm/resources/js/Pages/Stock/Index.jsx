                            ...
                            <div className="flex gap-1">
                                {items.links.map((link, i) => (
                                    <button key={i} disabled={!link.url}
                                        ...
                                        className={`px-3 py-1 rounded-lg text-xs font-medium transition-colors ${link.active ? 'text-white' : ''} ${!link.url ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'} ${isDark ? 'hover:bg-slate-800' : 'hover:bg-slate-100'}`}
