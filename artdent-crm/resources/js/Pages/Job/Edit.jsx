...
export default function Edit({ auth, item, dentists, patients, jobTypes, users }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        shade: item.shade || '',
        received_at: item.received_at ? dayjs(item.received_at).format('YYYY-MM-DD') : '',
        due_date: item.due_date ? dayjs(item.due_date).format('YYYY-MM-DD') : '',
        delivered_at: item.delivered_at ? dayjs(item.delivered_at).format('YYYY-MM-DD') : '',
        notes: item.notes || '',
    });
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title={`Editar Trabajo ${item.job_number}`} />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        ...
                        <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                            Actualizando registro del trabajo
                        </p>
                    </div>
