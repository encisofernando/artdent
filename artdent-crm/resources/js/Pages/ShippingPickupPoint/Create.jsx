...
export default function Create({ auth }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('shipping-pickup-points.store'));
    };
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Nuevo Punto de Retiro" />
            ...
            <form onSubmit={submit} className="flex flex-col gap-6 font-sans max-w-3xl mx-auto">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                        ...
                        <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                            Agregar un nuevo punto de entrega para los clientes
                        </p>
                    </div>
                ...
                <div className={section}>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div className="sm:col-span-2">
                            ...
                            <input className={inp} value={data.name} onChange={e => setData('name', e.target.value)} placeholder="Ej: Sucursal Centro" />
                            {errors.name && <p className={err}>{errors.name}</p>}
                        </div>

                        <div className="sm:col-span-2">
                            <label className={lbl}>Dirección *</label>
                            <input className={inp} value={data.address} onChange={e => setData('address', e.target.value)} placeholder="Calle y número" />
                            {errors.address && <p className={err}>{errors.address}</p>}
                        </div>

                        <div>
                            <label className={lbl}>Ciudad *</label>
                            <input className={inp} value={data.city} onChange={e => setData('city', e.target.value)} placeholder="Formosa" />
                            {errors.city && <p className={err}>{errors.city}</p>}
                        </div>

                        <div>
                            <label className={lbl}>Provincia *</label>
                            <input className={inp} value={data.province} onChange={e => setData('province', e.target.value)} placeholder="Formosa" />
                            {errors.province && <p className={err}>{errors.province}</p>}
                        </div>

                        <div>
                            <label className={lbl}>Código Postal</label>
                            <input className={inp} value={data.postal_code} onChange={e => setData('postal_code', e.target.value)} placeholder="3600" />
                        ...
                        <div className="sm:col-span-2">
                            ...
                            <input className={inp} value={data.schedule} onChange={e => setData('schedule', e.target.value)} placeholder="Lun-Vie 9:00–18:00 / Sáb 9:00–13:00" />
                ...
                <div className={section}>
                    <div className={`flex items-center gap-2 mb-6 pb-2 border-b ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
                        ...
                        <span className={`text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>(opcional)</span>
                    </div>

                    <p className={`text-xs mb-4 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                        Ingresá las coordenadas para mostrar el punto en el mapa durante el checkout.
                        Podés obtenerlas desde Google Maps haciendo clic derecho sobre el lugar.
                    </p>

                    <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                            <label className={lbl}>Latitud</label>
                            <input className={inp} type="number" step="any" value={data.latitude} onChange={e => setData('latitude', e.target.value)} placeholder="-26.1857" />
                            {errors.latitude && <p className={err}>{errors.latitude}</p>}
                        </div>
                        <div>
                            <label className={lbl}>Longitud</label>
                            <input className={inp} type="number" step="any" value={data.longitude} onChange={e => setData('longitude', e.target.value)} placeholder="-58.1756" />
                ...
                <div className={section}>
                    <div className="grid gap-4">
                        <div>
                            ...
                            <textarea className={inp} rows={3} value={data.notes} onChange={e => setData('notes', e.target.value)} placeholder="Información adicional..." />
