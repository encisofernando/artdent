...
export default function Index({ auth, items, employees, filters, summary }) {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (
                ...
                router.get(route('employee-receipts.index'), {
    ...
    }, [employeeFilter, statusFilter, filters?.employee_id, filters?.status]);

    const submitCreate = (e) => {
        e.preventDefault();
        createForm.post(route('employee-receipts.store'), {
            onSuccess: () => { setShowCreate(false); createForm.reset(); },
        });
    };

    const markPaid = (item) => {
        const name = item.employee?.user?.name ?? `Empleado #${item.employee_id}`;
        confirmDialog(`¿Marcar el recibo de ${name} como pagado?`, () =>
            router.put(route('employee-receipts.update', item.id), {
                status: 'paid',
                notes: item.notes,
            }, { preserveScroll: true })
        );
    ...
    const handleDelete = (id) => {
        confirmDialog('¿Eliminar este recibo?', () =>
            router.delete(route('employee-receipts.destroy', id), { preserveScroll: true })
        );
    };
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Recibos de Personal" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                    {[
                        ...
                        >
                            ...
                            <p className={`mt-2 text-2xl font-extrabold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{value}</p>
                        </div>
                    ))}
                </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead className={`border-b ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                                    <tr>
                                        {['Empleado', 'Período', 'Sueldo', 'Comisión', 'Extras', 'Descuentos', 'Neto', 'Estado', ...(showActions ? [''] : [])].map(h => (
                                    ...
                                    {data.map(item => (
                                        <tr key={item.id} className={`transition-colors ${isDark ? 'hover:bg-slate-800/40' : 'hover:bg-slate-50'}`}>
                                            ...
                                            <td className={`px-4 py-3 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                                                ${fmt(item.salary_gross)}
                                            </td>
                                            <td className={`px-4 py-3 font-bold ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>
                                                {parseFloat(item.commission_gross) > 0 ? `$${fmt(item.commission_gross)}` : '—'}
                                            </td>
                    ...
                    <form onSubmit={submitCreate}>
                        <div className="flex flex-col gap-4">
                            <div>
                                ...
                                <SearchableSelect
                                    value={String(createForm.data.employee_id || '')}
                                    onChange={v => createForm.setData('employee_id', v)}
                                    placeholder="Seleccionar..."
                                    options={employees.map(e => ({ value: String(e.id), label: e.user?.name ?? `Empleado #${e.id}` }))}
                                />
                                {createForm.errors.employee_id && <p className="text-red-500 text-xs mt-1">{createForm.errors.employee_id}</p>}
                            </div>
                            ...
                            <p className={`text-xs ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                                Se calcularán automáticamente el sueldo, comisiones, extras y descuentos del período seleccionado.
            </p>
                        </div>
